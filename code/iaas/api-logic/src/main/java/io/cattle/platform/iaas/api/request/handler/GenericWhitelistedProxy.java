...
public class GenericWhitelistedProxy extends AbstractResponseGenerator implements Named {
    ...
    protected void generate(final ApiRequest request) throws IOException {
        ...
        case "DELETE":
        case "HEAD":
            temp = new JdkProxyRequest(method, uri, targetPolicy(servletRequest, uri));
            break;
        default:
    ...
    static boolean isProxyableUri(URI uri) {
        return uri != null && isProxyableScheme(uri.getScheme()) && StringUtils.isNotBlank(uri.getHost())
                && uri.getUserInfo() == null && uri.getFragment() == null;
    }

    Predicate<URI> targetPolicy(HttpServletRequest servletRequest, URI initialTarget) {
        boolean trustedInternalRoute = Boolean.TRUE.equals(servletRequest.getAttribute(ALLOWED_HOST));
        return candidate -> {
            if (!sameOrigin(initialTarget, candidate)) {
                return false;
            }
            if (trustedInternalRoute) {
                return true;
            }
            String candidateHost = candidate.getPort() > 0
                    ? String.format("%s:%s", candidate.getHost(), candidate.getPort()) : candidate.getHost();
            if (!isAllowed(servletRequest, candidateHost)) {
                return false;
            }
            if (!isNoAuthProxy()) {
                return true;
            }
            if (candidate.getPath() == null || allowedPaths == null) {
                return false;
            }
            for (String path : allowedPaths) {
    ...
    static boolean sameOrigin(URI first, URI second) {
        if (!isProxyableUri(first) || !isProxyableUri(second)) {
            return false;
        }
