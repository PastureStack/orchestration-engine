...
public class JdkProxyRequestTest {
    ...
    public void forwardsManagedProxyHeadersAndUsesHttp11() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("POST", url("/headers"));
    ...
    public void streamsRawBodyWithoutReEncoding() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("PUT", url("/raw"));
    ...
    public void encodesLegacyFormBodyOnlyWhenRequested() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("POST", url("/form"));
    ...
    public void doesNotFollowRedirectsWhenDisabled() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("GET", url("/redirect"));
        JdkProxyRequest.ProxyResponse response = request.execute(false);
    ...
    @Test
    public void followsSameOriginRedirectWhenHostCaseChanges() throws Exception {
        AtomicBoolean targetHit = new AtomicBoolean(false);

        startServer(exchange -> {
            if ("/redirect".equals(exchange.getRequestURI().getPath())) {
                exchange.getResponseHeaders().set("Location", "HTTP://LOCALHOST:"
        ...
        URI initial = URI.create("http://localhost:" + server.getAddress().getPort() + "/redirect");
        JdkProxyRequest request = new JdkProxyRequest("GET", initial, candidate -> true);
        JdkProxyRequest.ProxyResponse response = request.execute(true);

        assertEquals(200, response.getStatusCode());
        assertEquals("target", new String(readAll(response.getBody()), StandardCharsets.UTF_8));
        assertTrue(targetHit.get());
    }

    @Test
    public void doesNotReplayStreamingBodyAcrossTemporaryRedirect() throws Exception {
        AtomicBoolean targetHit = new AtomicBoolean(false);

        startServer(exchange -> {
            if ("/redirect".equals(exchange.getRequestURI().getPath())) {
                exchange.getResponseHeaders().set("Location", url("/target"));
                write(exchange, 307, bytes("redirect"));
                return;
            }
            targetHit.set(true);
            write(exchange, 200, bytes("unexpected"));
        });

        JdkProxyRequest request = allowedRequest("PUT", url("/redirect"));
        request.body(new ByteArrayInputStream(bytes("one-shot")), 8);
        JdkProxyRequest.ProxyResponse response = request.execute(true);

        assertEquals(307, response.getStatusCode());
        assertEquals("redirect", new String(readAll(response.getBody()), StandardCharsets.UTF_8));
        assertFalse(targetHit.get());
    }
    ...
    public void preservesResponseHeadersAndStreamsLargeBody() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("GET", url("/large"));
        JdkProxyRequest.ProxyResponse response = request.execute(false);
    ...
    public void honorsRequestTimeout() throws Exception {
        ...
        JdkProxyRequest request = allowedRequest("GET", url("/slow"));
    ...
    @Test
    public void blocksCrossOriginRedirectEvenWhenExternalPolicyAllowsIt() throws Exception {
        AtomicBoolean secondServerHit = new AtomicBoolean(false);
        HttpServer secondServer = HttpServer.create(new InetSocketAddress(InetAddress.getLoopbackAddress(), 0), 0);
        ...
        try {
            startServer(exchange -> {
                String target = "http://" + secondServer.getAddress().getHostString() + ":"
                        + secondServer.getAddress().getPort() + "/private";
                exchange.getResponseHeaders().set("Location", target);
                write(exchange, 302, bytes("redirect"));
            });

            URI initial = URI.create(url("/redirect"));
            JdkProxyRequest request = new JdkProxyRequest("GET", initial, candidate -> true);
            try {
                request.execute(true);
                fail("Expected redirect policy rejection");
            } catch (IOException e) {
                assertTrue(e.getMessage().contains("not allowed"));
            }
            assertFalse(secondServerHit.get());
        } finally {
            secondServer.stop(0);
        }
    }
    ...
    public void rejectsDisallowedInitialTarget() {
        new JdkProxyRequest("GET", URI.create("http://169.254.169.254/latest/meta-data"), candidate -> false);
    }

    private JdkProxyRequest allowedRequest(String method, String target) {
        return new JdkProxyRequest(method, URI.create(target), candidate -> true);
    }
