...
public class GenericWhitelistedProxyTest {
    ...
    @Test
    public void sameOriginRequiresSchemeHostAndEffectivePort() {
        assertTrue(GenericWhitelistedProxy.sameOrigin(
                URI.create("https://catalog.example/path"), URI.create("https://catalog.example:443/next")));
        assertFalse(GenericWhitelistedProxy.sameOrigin(
                URI.create("https://catalog.example/path"), URI.create("http://catalog.example/next")));
        assertFalse(GenericWhitelistedProxy.sameOrigin(
                URI.create("https://catalog.example/path"), URI.create("https://evil.example/next")));
        assertFalse(GenericWhitelistedProxy.sameOrigin(
                URI.create("https://catalog.example/path"), URI.create("https://catalog.example:8443/next")));
    }

    @Test
    public void proxyUriRejectsUserInfoAndFragments() {
        assertFalse(GenericWhitelistedProxy.isProxyableUri(URI.create("http://trusted.example@evil.example/path")));
        assertFalse(GenericWhitelistedProxy.isProxyableUri(URI.create("https://trusted.example/path#fragment")));
        assertTrue(GenericWhitelistedProxy.isProxyableUri(URI.create("https://trusted.example/path")));
    }
