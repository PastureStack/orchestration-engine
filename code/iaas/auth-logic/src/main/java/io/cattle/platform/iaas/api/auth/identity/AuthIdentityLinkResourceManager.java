...
public class AuthIdentityLinkResourceManager extends AbstractNoOpResourceManager {
    ...
    private Resource bind(Account actor, Map<String, Object> input) {
        ...
        if (preparesProviderSwitch(input)) {
            verifyLocalAdministratorRecovery(input);
        }
    ...
    private Resource switchToLocal(Map<String, Object> input) {
        ...
        if (!mfaService.hasPrimaryFactor(localAccount.getId())) {
            throw new ClientVisibleException(ResponseCodes.CONFLICT,
                    "LocalAdministratorMfaRequired",
                    "Register an authenticator app or passkey for the local system administrator "
                            + "before switching authentication providers.",
                    null);
        }
        ...
        // completes platform MFA, and then establishes a session for the
        // verified local administrator.
        ...
        settingsUtils.changeSetting(LocalAuthConstants.RECOVERY_MFA_READY_SETTING, true);
    ...
    private boolean preparesProviderSwitch(Map<String, Object> input) {
        return BooleanUtils.toBoolean(string(input.get("prepareProviderSwitch")));
    }

    private void verifyLocalAdministratorRecovery(Map<String, Object> input) {
        String username = string(input.get("localUsername"));
        String password = string(input.get("localPassword"));
        if (StringUtils.isAnyBlank(username, password)) {
            throw new ClientVisibleException(ResponseCodes.BAD_REQUEST, "LocalCredentialsRequired",
                    "Verify an existing local system-administrator username and password before "
                            + "activating an external authentication provider.",
                    null);
        }
        Account localAccount = authDao.getAccountByLogin(username, password,
                ApiContext.getContext().getTransformationService());
        ...
        if (!mfaService.hasPrimaryFactor(localAccount.getId())) {
            throw new ClientVisibleException(ResponseCodes.CONFLICT,
                    "LocalAdministratorMfaRequired",
                    "Register an authenticator app or passkey for the local system administrator "
                            + "before activating an external authentication provider.",
                    null);
        }
        settingsUtils.changeSetting(SecurityConstants.SECURITY_SETTING, true);
        settingsUtils.changeSetting(LocalAuthConstants.RECOVERY_ENABLED_SETTING, true);
