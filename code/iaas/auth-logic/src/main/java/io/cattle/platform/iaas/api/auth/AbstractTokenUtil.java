...
public abstract class AbstractTokenUtil implements TokenUtil {
    ...
    /**
     * Recognizes the emergency local-administrator session after an external
     * provider has become active.  The encrypted payload type is generated by
     * the local token creator, while the durable token record is still bound
     * to the active provider by {@link #retrieveJwt(String)}.  Token issuance
     * has already required platform MFA; rechecking recovery state, principal
     * activity, and administrator kind here prevents the external provider's
     * required allow-list from locking out the documented recovery path.
     */
    protected boolean isVerifiedLocalRecoveryToken(Map<String, Object> jsonData) {
        if (jsonData == null
                || !LocalAuthConstants.JWT.equalsIgnoreCase(
                        java.util.Objects.toString(jsonData.get(TOKEN), ""))
                || !Boolean.TRUE.equals(SecurityConstants.SECURITY.get())
                || !Boolean.TRUE.equals(LocalAuthConstants.RECOVERY_ENABLED.get())) {
            return false;
        }
        Account account = accountFromPrincipalClaim(jsonData.get(PRINCIPAL_ACCOUNT_ID));
        return account != null
