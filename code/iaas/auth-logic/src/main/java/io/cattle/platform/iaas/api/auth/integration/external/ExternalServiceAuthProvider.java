...
public class ExternalServiceAuthProvider {
    ...
    public Token getToken(ApiRequest request) {
        ...
        try {
            ...
            token.setFederatedAuthenticationMethods(stringList(
                    decryptedToken.get("authentication_methods")));
            token.setFederatedAuthenticationContext(stringOrNull(
                    decryptedToken.get("authentication_context")));
            token.setFederatedAuthenticatedAt(longObject(
                    decryptedToken.get("authenticated_at")));
            token.setFederatedAuthenticationIssuer(stringOrNull(
                    decryptedToken.get("authentication_issuer")));
            return token;
        } catch (IOException e) {
    ...
    public Identity getIdentity(String id, String scope) {
        ...
        if(ServiceAuthConstants.NO_IDENTITY_LOOKUP_SUPPORTED.get()) {
            ...
            if (tokenUtil.findAndSetJWT()) {
                ...
                log.debug("Resolved {} identities from the authentication token", identitiesInToken.size());
    ...
    private List<String> stringList(Object value) {
        ...
        List<?> items = CollectionUtils.toList(value);
        if (items == null) {
            return result;
        }
