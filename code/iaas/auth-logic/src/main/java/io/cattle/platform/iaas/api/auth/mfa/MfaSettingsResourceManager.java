...
public class MfaSettingsResourceManager extends AbstractNoOpResourceManager {
    ...
    protected Object getByIdInternal(String type, String id, ListOptions options) {
        if (!TYPE.equalsIgnoreCase(type)
                || !MfaPolicyService.GLOBAL_CONFIG_KEY.equals(id)) {
            return null;
        }
        requireAdministrator();
        return resource(policyService.getConfigCredential(), "ready");
    }
    ...
    protected Object updateInternal(String type, String id, Object obj, ApiRequest request) {
        ...
        return createInternal(type, request);
    }
                ...
                MfaPolicyService.GLOBAL_CONFIG_KEY), new LockCallback<Resource>() {
                    ...
                    public Resource doWithLock() {
                        ...
                        merge(data, input, "enforcement", "passkeyLimit", "relyingPartyId", "origin",
                                ...
                                "emailCodeTtlSeconds", "maximumFailedAttempts",
                        ...
                        MfaPolicy candidate = new MfaPolicy(string(data.get("enforcement")),
                                ...
                                integer(data.get("maximumFailedAttempts"), 10),
                                integer(data.get("lockoutSeconds"), 300),
                                integer(data.get("securityConfirmationTtlSeconds"), 300),
                                string(data.get("federatedMfaMode")),
                                string(data.get("trustedAuthenticationMethods")),
                                string(data.get("trustedAuthenticationContexts")),
                                integer(data.get("maximumFederatedAuthenticationAgeSeconds"), 300),
                                string(data.get("passkeyCounterPolicy")),
                                string(data.get("securityEmailLocale")));
    ...
    private Resource resource(Credential credential, String status) {
        ...
        fields.put("maximumFailedAttempts", policy.getMaximumFailedAttempts());
        fields.put("lockoutSeconds", policy.getLockoutSeconds());
        fields.put("securityConfirmationTtlSeconds",
                policy.getSecurityConfirmationTtlSeconds());
        fields.put("federatedMfaMode", policy.getFederatedMfaMode());
        fields.put("trustedAuthenticationMethods",
                StringUtils.join(policy.getTrustedAuthenticationMethods(), ","));
        fields.put("trustedAuthenticationContexts",
                StringUtils.join(policy.getTrustedAuthenticationContexts(), ","));
        fields.put("maximumFederatedAuthenticationAgeSeconds",
                policy.getMaximumFederatedAuthenticationAgeSeconds());
        fields.put("passkeyCounterPolicy", policy.getPasskeyCounterPolicy());
        fields.put("securityEmailLocale", policy.getSecurityEmailLocale());
        boolean externalProviderActive = SecurityConstants.SECURITY.get()
                && !LocalAuthConstants.CONFIG.equalsIgnoreCase(
                SecurityConstants.AUTH_PROVIDER.get());
        fields.put("localAdministratorRecoveryRequired", externalProviderActive);
        boolean localRecoveryEnabled = Boolean.TRUE.equals(
                LocalAuthConstants.RECOVERY_ENABLED.get());
        boolean localRecoveryMfaReady = Boolean.TRUE.equals(
                LocalAuthConstants.RECOVERY_MFA_READY.get());
        fields.put("localAdministratorRecoveryConfigured", localRecoveryEnabled);
        fields.put("localAdministratorRecoveryMfaReady", localRecoveryMfaReady);
        fields.put("localAdministratorRecoveryEnabled",
                localRecoveryEnabled && localRecoveryMfaReady);
        fields.put("localAdministratorRecoveryStatus",
                !externalProviderActive || (localRecoveryEnabled && localRecoveryMfaReady)
                        ? "ready" : "blocked");
