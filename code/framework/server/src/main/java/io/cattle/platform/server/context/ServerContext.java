...
public class ServerContext {
    ...
    public static String getServerId() {
        ...
        if (id != null) {
            return applyServerIdTemplate(id, ip);
        }

        return applyServerIdTemplate(SERVER_ID_FORMAT, ip);
    }

    static String applyServerIdTemplate(String template, String ip) {
        if (template == null || template.length() > 256) {
            throw new IllegalArgumentException("Invalid server ID template");
        }

        StringBuilder result = new StringBuilder(template.length() + ip.length());
        ...
        for (int i = 0; i < template.length(); i++) {
            ...
            if (next == '%') {
                result.append('%');
            } else if (next == 's' && !placeholderFound) {
                result.append(ip);
                placeholderFound = true;
            } else {
                throw new IllegalArgumentException("Server ID template supports one %s placeholder");
            }
        }
