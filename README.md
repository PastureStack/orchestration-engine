...
Container and service volume changes expose a read-only `volumepreflight` project action. Project authorization explicitly preserves the complete action input, read-only result, and issue schemas so the browser can perform the same storage-driver, path, access-mode, eligible-host, and `pasturestack-nfs` coverage checks enforced again during create and upgrade. A packaging regression test loads the shipped user and project authorization overlays and fails if any action schema or nested field becomes unavailable.
...
```sh
ENGINE_VERSION=0.183.280 bash scripts/build --release
